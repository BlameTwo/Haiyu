...
{
    public class CreateUninstallSetup : ISetup
    {
        public string SetupName => "创建卸载方式";
        ...
        )
        {
            return await Task.Run(() =>
            {
                try
                {
                    string targetPath = property.UninstallString;
                    string shortcutName = property.InstallExeName;
                    var startMenuFolder = property.GetStartMenuFolder();
                    string startMenuExeLine = property.GetUninstallLink();
